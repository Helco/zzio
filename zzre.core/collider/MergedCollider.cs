...
public sealed class MergedCollider : TreeCollider<Box>
{
    ...
    public static MergedCollider CreateDF1(RWWorld world)
    {
    ...
    public static MergedCollider CreateDF2(RWWorld world)
    {
        var atomicSections = world
            .FindAllChildrenById(SectionId.AtomicSection, recursive: true)
            .Cast<RWAtomicSection>()
        ...
        var splits = new CollisionSplit[splitCount];
        var splitI = 0;
        ProcessSubSection(default, 0f, rootSection);
        ...
        CollisionSector ProcessSubSection(CollisionSectorType parentSplitType, float value, Section section)
        {
            switch (section)
            {
                case RWPlaneSection subPlane:
                    var subPlaneSplitI = splitI++;
                    var leftSection = subPlane.children[0];
                    var rightSection = subPlane.children[1];
                    var splitType = subPlane.sectorType switch
                    {
                        RWPlaneSectionType.XPlane => CollisionSectorType.X,
                        RWPlaneSectionType.YPlane => CollisionSectorType.Y,
                        RWPlaneSectionType.ZPlane => CollisionSectorType.Z,
                    ...
                    splits[subPlaneSplitI] = new()
                    {
                        left = ProcessSubSection(splitType, subPlane.leftValue, leftSection),
                        right = ProcessSubSection(splitType, subPlane.rightValue, rightSection)
                    };
    ...
    public static MergedCollider CreateBF(RWWorld world)
    {
        var atomicSections = world
            .FindAllChildrenById(SectionId.AtomicSection, recursive: true)
            .Cast<RWAtomicSection>()
        ...
        var splits = new CollisionSplit[splitCount];
        var splitI = 1;
        var planeStack = new Queue<(RWPlaneSection, int)>();
        planeStack.Enqueue((rootPlane!, 0));
        while (planeStack.TryDequeue(out var tuple))
        {
            var (plane, planeSplitI) = tuple;
            var leftSection = plane.children[0];
            var rightSection = plane.children[1];
        ...
        CollisionSector ProcessSubSection(CollisionSectorType splitType, float value, Section section)
        {
            switch (section)
            {
                case RWPlaneSection subPlane:
                    var subPlaneSplitI = splitI++;
                    planeStack.Enqueue((subPlane, subPlaneSplitI));
                    return new()
                    {
                        type = splitType,
                        value = value,
